...
export default function BrowsePage() {
  ...
  async function fetchListings() {
    try {
      const { data } = await axios.get(`${API_BASE}/api/listings`);
  ...
  async function fetchUser() {
    try {
      const { data } = await axios.get(`${API_BASE}/api/user1`);
  ...
  function applyFilters() {
    ...
    setListings(
      originalListings.filter(listing => canReserve(listing))
    );
  }
